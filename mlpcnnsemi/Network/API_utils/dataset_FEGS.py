...
class API_FEGS_Class(torch.utils.data.Dataset):
    def __init__(self,xls_path,fegs_pth,name):

        api_dict = {}
        
        df = pd.read_excel(xls_path)
        count = 0
        mat = sio.loadmat(fegs_pth)
        # print(mat)
        mat = mat[name]
        for index,row in df.iterrows():
            ...
            temp_dir["FEGS"] = mat[temp_dir["id"]-1]
            if(row["class"]=="positive"):
                temp_dir["class"] = 1 ## Original is 1
                # temp_dir["class"] = np.array([1,0]) ## Original is 1
            else:
                temp_dir["class"] = 0
                # temp_dir["class"] = np.array([0,1])
    ...
    def __getitem__(self,index):
        ...
        pro_sequence = temp_dir["FEGS"]
        
        list_apt = count_kmers(apt_sequence)
        list_pro = list(pro_sequence)
